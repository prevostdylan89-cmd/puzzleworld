...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch all user_puzzles from Firestore
    let allDocs = [];
    let pageToken = null;
    do {
    ...
    // Extract unique puzzles by ASIN or by title+brand combo
    const seen = new Map();
    for (const doc of allDocs) {
      ...
      const ean = val(f.ean) || '';
      const catalogId = val(f.catalog_id) || val(f.puzzle_catalog_id) || '';

      if (!title) continue;

      // Use ASIN as key if available, otherwise title+brand
    ...
    // Sample the first 5 for inspection
    const sample = uniquePuzzles.slice(0, 5);

    if (dryRun) {
      return Response.json({
        dryRun: true,
        totalUserPuzzleDocs: allDocs.length,
        uniquePuzzlesFound: uniquePuzzles.length,
        sample,
        message: `DRY RUN: ${uniquePuzzles.length} puzzles uniques trouvés dans ${allDocs.length} docs user_puzzles. Relance avec dryRun:false pour importer.`
      });
    }

    // Get existing catalog to avoid duplicates
    ...
    const existingTitles = new Set(existing.map(p => `${p.title}__${p.brand}__${p.piece_count}`));

    let added = 0;
    let skipped = 0;
    for (const puzzle of uniquePuzzles) {
      const titleKey = `${puzzle.title}__${puzzle.brand}__${puzzle.piece_count}`;
      if ((puzzle.asin && existingAsins.has(puzzle.asin)) || existingTitles.has(titleKey)) {
        skipped++;
        continue;
      }
      const { _catalogId, ...puzzleData } = puzzle;
      await base44.asServiceRole.entities.PuzzleCatalog.create(puzzleData);
      added++;
    }
    ...
    return Response.json({
      ...
      added,
      skipped,
      message: `${added} puzzles ajoutés au catalogue (${skipped} déjà présents).`
    });
